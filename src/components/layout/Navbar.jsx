  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isHome && !scrolled
          ? "bg-transparent py-5"
          : "bg-slate-950/95 backdrop-blur-xl shadow-lg py-3"
      }`}
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-xs leading-tight text-slate-200">CNET Community · Chennai</p>
            </div>
          </Link>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                  pathname === link.href ? "text-accent" : "text-slate-200 hover:text-white"
                }`}
              >
                {link.label}
              </Link>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <Link
              href="/join"
              className="rounded-full bg-accent px-5 py-2.5 text-sm font-semibold text-slate-950 hover:bg-accent-dark transition-all duration-200"
            >
              Join Now
            </Link>
          </div>
        ...
        {isOpen && (
          <div className="lg:hidden mt-3 pb-4 border-t border-slate-100 animate-slide-down">
            <nav className="flex flex-col gap-1 mt-3">
              ...
              <Link href="/membership" className="mt-2 btn-primary text-sm text-center">
                Join Us
              </Link>
            </nav>
          </div>
